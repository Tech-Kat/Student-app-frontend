...
const StudentList = ({ studentData }) => {
  // when i type in the input i hould see filtered results by name
  ...
  //reassign datadislay with the filtered data

  if (searchInput) {
    //we only reach this code if the user has typed something
    dataToDisplay = studentData.filter((student) => {
      //If students fullname includes the searchInput , return true
      //otherwise false
      //case insensitive
  ...
  return (
    <div className="StudentList">
      ...
      <input
        // value will correspond to state
        value={searchInput}
        type="text"
